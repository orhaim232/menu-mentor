...
### משימה 15: רשימת מנות במסך מנהל
1. **מטרה:** בניית ממשק המציג את כל המנות הקיימות והקטגוריות שלהן במסך `app/manager/setup/page.tsx`.
2. **זמן:** 30 דקות.

### משימה 16: טופס הוספה/עריכה ידנית
1. **מטרה:** בניית טופס למנהל המאפשר יצירת ועריכת מנה, בחירת קטגוריה, והזנת רכיבים/אלרגנים/תגיות. `custom_attributes` (כמו spice_level) ינוהלו כרגע בסיסית.
2. **זמן:** 30 דקות.

### משימה 17: תצוגה מקדימה של verified_demo_menu.json
1. **מטרה:** טעינת הקובץ לזיכרון ב-Client, הצגת טבלה חכמה עם אזהרות (חסר אימות), ושדות שיומרו ל-`custom_attributes`.
2. **זמן:** 30 דקות.

### משימה 18: אישור וייבוא ל-Supabase
1. **מטרה:** כפתור ששומר המונית (Bulk Insert) את טיוטת הייבוא ל-DB, מייצר קטגוריות חסרות ומקדם `menu_version`.
2. **זמן:** 30 דקות.
